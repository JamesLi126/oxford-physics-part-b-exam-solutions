...
\[
-\frac{(a^r)^2 (u^r)^2}{f^3 (u^0)^2} + \frac{(a^r)^2}{f} = g^2.
\]
Factor $f^{-1}(a^r)^2$:
\[
\frac{(a^r)^2}{f}\left[1 - \frac{(u^r)^2}{f^2\,(u^0)^2}\right] = g^2.
\]
Multiply numerator and denominator inside the bracket by $f$, and insert $f(u^0)^2 = c^2 + f^{-1}(u^r)^2$ from (d):
\[
f^2 (u^0)^2 = f\bigl[c^2 + f^{-1}(u^r)^2\bigr] = f c^2 + (u^r)^2.
\]
Simplify the bracket:
\[
1 - \frac{(u^r)^2}{f c^2 + (u^r)^2} = \frac{f c^2}{f c^2 + (u^r)^2}.
\]
Substitute back:
\[
\frac{(a^r)^2}{f}\cdot \frac{f c^2}{f c^2 + (u^r)^2} = g^2.
\]
Cancel $f$:
\[
\frac{(a^r)^2 c^2}{f c^2 + (u^r)^2} = g^2.
\]
Solve for $(a^r)^2$:
\[
(a^r)^2 = g^2\,\frac{f c^2 + (u^r)^2}{c^2} = g^2\left[f + \frac{(u^r)^2}{c^2}\right].
\]
...
\]
Evaluate the Newtonian term at Isaac's radius $r^2 = GM/g$:
\[
\frac{GM}{r^2} = \frac{GM}{GM/g} = g.
\]
Plug into the escape inequality:
\[
g\sqrt{1 - \frac{2GM}{c^2 r}} \ge g.
\]
Cancel $g$ and require the square root to be $\ge 1$:
\[
\sqrt{1 - \frac{2GM}{c^2 r}} \ge 1.
\]
Since $2GM/(c^2 r)>0$ for finite $r$, the left-hand side is strictly less than $1$:
\[
...
\]
Raise both indices via $T^{\alpha\beta} = g^{\alpha\gamma}g^{\beta\delta}T_{\gamma\delta}$ (diagonal):
\[
T^{\alpha\beta} = -\frac{c^4}{8\pi G}\,G^{\alpha\beta},\qquad G^{\alpha\beta} = g^{\alpha\alpha}g^{\beta\beta}G_{\alpha\beta}.
\]
Work in the basis $x^0 = ct$, so $g_{00}=-1$, $g^{00}=-1$, and $G_{00}=G_{tt}/c^2 = f''/f$:
\[
G^{00} = (g^{00})^2 G_{00} = \frac{f''}{f},\quad G^{rr}=0,\quad G^{\phi\phi}=0,\quad G^{zz} = -\frac{f''}{f}.
\]
Substitute:
\[
T^{00} = -\frac{c^4}{8\pi G}\cdot\frac{f''}{f} = -\frac{c^4 f''}{8\pi G f},\qquad T^{zz} = +\frac{c^4 f''}{8\pi G f}.
\]
Collect with all four components:
\[
...
Energy per unit length $\mu$ is the integral of the energy density over the proper transverse cross-section. Proper area element on a $t,z=$ const slice is $\de A = f(r)\,\de r\,\de\phi$. The energy density (component $T^{00}$ in the orthonormal frame $x^0=ct$) is $\rho c^2 = c^4 f''/(8\pi G f)$:
\[
\mu c^2 = \int_0^{2\pi}\!\de\phi\int_0^{R}\!\rho c^2\,f(r)\,\de r.
\]
Substitute $\rho c^2$:
\[
\mu c^2 = \int_0^{2\pi}\!\de\phi\int_0^{R}\!\frac{c^4 f''(r)}{8\pi G f(r)}\,f(r)\,\de r.
\]
Cancel $f$ in the integrand:
\[
\mu c^2 = \frac{c^4}{8\pi G}\int_0^{2\pi}\!\de\phi\int_0^{R}\! f''(r)\,\de r.
\]
Integrate trivially in $\phi$ and once in $r$:
\[
\mu c^2 = \frac{c^4}{8\pi G}\cdot 2\pi\bigl[f'(R) - f'(0)\bigr] = \frac{c^4}{4G}\bigl[f'(R) - f'(0)\bigr].
\]
Divide by $c^2$:
\[
...
\textbf{Boundary values.} The problem states $f'(r)\to 0$ as $r\to 0$, so $f'(0)=0$.

\textbf{Matching at $r=R$.} Both $f$ and $f'$ are assumed continuous at $r=R$ (so that the metric and its first derivatives, hence the Christoffels, are continuous). For $r>R$ the line element gives $f(r) = (1-\lambda)r$, hence $f'(r) = 1-\lambda$:
\[
f'(R) = 1-\lambda.
\]
Substitute into \eqref{eq:mu-general}:
\[
\boxed{\;\mu = \frac{c^2}{4G}(1-\lambda).\;}
\label{eq:mu-cosmic}
\]
...
The scenario: we send a message now at $t_0$; it reaches them at $t_1$; they reply immediately; their reply reaches us at $t_2$. We want the wait $t_2 - t_0$.

The galaxy currently appears at $z=1$, so light reaching us now was emitted at $t_e$ with $R(t_e)/R(t_0) = 1/(1+z) = 1/2$. With $R\propto t^{2/3}$:
\[
t_e = t_0\,(1/2)^{3/2} = \frac{t_0}{2\sqrt{2}}.
\]

The galaxy's fixed comoving distance from us is set by this past-light null geodesic:
\[
\chi = \int_{t_e}^{t_0}\frac{c\,\de t}{R(t)} = \frac{c}{R_0}\int_{t_e}^{t_0}\!\left(\frac{t_0}{t}\right)^{2/3}\!\de t.
\]
Integrate $t^{-2/3}$:
\[
\chi = \frac{c\, t_0^{2/3}}{R_0}\cdot 3\bigl[t_0^{1/3} - t_e^{1/3}\bigr] = \frac{3 c\, t_0}{R_0}\bigl[1 - (t_e/t_0)^{1/3}\bigr].
\]
Plug $(t_e/t_0)^{1/3} = (2^{-3/2})^{1/3} = 1/\sqrt 2$:
\[
\chi = \frac{3 c\, t_0}{R_0}\left(1 - \tfrac{1}{\sqrt 2}\right).
\]

Define $\tau\equiv(t/t_0)^{1/3}$; the comoving distance covered between times $t_a, t_b$ ($t_a<t_b$) is $(3ct_0/R_0)(\tau_b - \tau_a)$.

\textbf{Outgoing leg: $t_0 \to t_1$.} Same $\chi$:
\[
\tau_1 - 1 = 1 - \frac{1}{\sqrt 2}\quad\Longrightarrow\quad \tau_1 = 2 - \frac{1}{\sqrt 2}\approx 1.293.
\]
Cube:
\[
t_1/t_0 \approx 2.161.
\]

\textbf{Return leg: $t_1 \to t_2$.} Same $\chi$ again:
\[
\tau_2 - \tau_1 = 1 - \frac{1}{\sqrt 2}\quad\Longrightarrow\quad \tau_2 = \tau_1 + 1 - \frac{1}{\sqrt 2}.
\]
Substitute $\tau_1$:
\[
\tau_2 = 3 - \frac{2}{\sqrt 2} = 3 - \sqrt 2\approx 1.586.
\]
Cube:
\[
t_2/t_0 \approx 3.99.
...
Round-trip wait:
\[
\Delta t_{\text{round}} = t_2 - t_0 \approx 2.99\,t_0.
\]
Insert $t_0 = 2/(3H_0)$:
\[
\boxed{\;\Delta t_{\text{round}} \approx 2.99 \times \tfrac{2}{3 H_0} \approx \frac{1.99}{H_0}.\;}
\]
With $H_0 = \SI{72}{km\,s^{-1}\,Mpc^{-1}}$, $H_0^{-1} \approx \SI{1.36e10}{yr}$:
\[
\Delta t_{\text{round}} \approx 1.99 \times \SI{1.36e10}{yr} \approx \SI{2.7e10}{yr}.
\]
...
\]
Simplify the prefactor $g_i \cdot 4\pi/(2\pi\hbar)^3 = g_i/(2\pi^2\hbar^3)$:
\[
N_i = V\,\frac{g_i}{2\pi^2}\left(\frac{k_B T}{\hbar c}\right)^3\!\!\int_0^\infty\frac{x^2\,\de x}{e^x \pm 1}.
\]
Collect:
\[
\boxed{\;\frac{N_i}{V} = \frac{g_i\,\zeta(3)}{\pi^2}\left(\frac{k_B T}{\hbar c}\right)^3\times\begin{cases}1 & \text{boson}\\ 3/4 & \text{fermion}\end{cases}.\;}
\label{eq:Ni}
\]
...
\textbf{Number density per neutrino species (today).} Apply the fermion form of \eqref{eq:Ni}. Per flavour, count one helicity of $\nu$ and one of $\bar\nu$, giving $g_{\nu+\bar\nu} = 2$:
\[
n_\nu^{\rm flavour} = \frac{g_{\nu+\bar\nu}\,\zeta(3)}{\pi^2}\cdot\frac{3}{4}\left(\frac{k_B T_{\nu,0}}{\hbar c}\right)^3 = \frac{3\zeta(3)}{2\pi^2}\left(\frac{k_B T_{\nu,0}}{\hbar c}\right)^3.
\]
...
\[
\frac{3\zeta(3)}{2\pi^2} = \frac{3\times 1.202}{2\pi^2} = \frac{3.606}{19.74} = 0.1827.
\]
Multiply:
\[
n_\nu^{\rm flavour} = 0.1827 \times \SI{6.16e8}{m^{-3}} \approx \SI{1.12e8}{m^{-3}} \approx \SI{112}{cm^{-3}}.
\]
Three flavours:
\[
