...
u_0''=\frac{i\omega}{\nu}u_0.
\]
Roots $\pm k$ with $k=\sqrt{i\omega/\nu}=(1+i)\sqrt{\omega/(2\nu)}$. Decay as $z\to\infty$ kills $e^{+kz}$:
\[
u_0(z)=U e^{-kz}.
\]
Apply $u_x(0,t)=U\cos\omega t$, fixing the amplitude. Define $\delta=\sqrt{2\nu/\omega}$:
\begin{equation}
...
\]
From the general expansion keep $n=1$ modes only:
\[
...
A_1=-U,\qquad C_1=0.
\]
Apply $\partial_r\phi|_a=A\sin\theta$:
\[
...
\]
Combine $A^2\sin^2\theta+A^2\cos^2\theta=A^2$ and $2\sin\theta\cos\theta=\sin 2\theta$:
\[
|\vb u|^2=A^2+4U^2\sin^2\theta-2UA\sin 2\theta.
\]
Steady Bernoulli on the level $z=$const around the cylinder (axis vertical, gravity drops):
\[
p+\tfrac12\rho|\vb u|^2=p_0+\tfrac12\rho U^2.
\]
Solve for $p(a,\theta)$:
\[
...
\[
p(a,\theta)=p_0+\tfrac12\rho U^2-\tfrac12\rho A^2-\rho U^2(1-\cos 2\theta)+\rho UA\sin 2\theta.
\]
...
Active case $A=U$, upper surface $\theta>0$. Pressure gradient:
\[
\frac{1}{a}\pd{p}{\theta}=\rho U^2\!\left[-2\sin 2\theta+2\cos 2\theta\right]\frac{1}{a}.
\]
Adverse where $\cos 2\theta<\sin 2\theta$, i.e.\ $\theta>\pi/8$. Compared with $A=0$ the adverse-pressure region begins later in the inviscid pressure pattern \emph{but} the new minimum is at $5\pi/8$, past the top. More importantly the imposed $u_r=A\sin\theta>0$ blows fluid \emph{outward} from the wall on $\theta>0$, energising the boundary layer (analogous to suction-blowing control). The active blowing therefore \emph{inhibits} separation on the upper surface relative to $A=0$.
...
Apply continuity of $\hat\psi$:
\[
C_+=C_-\equiv C.
\]
Evaluate the derivative jump. At $y=0^+$: $\hat\psi'=-kC$, $\overline U'=-\alpha$. At $y=0^-$: $\hat\psi'=+kC$, $\overline U'=0$. Insert into the second condition:
\[
\left[\frac{\omega}{k}(-kC)+C(-\alpha)\right]-\left[\frac{\omega}{k}(+kC)+0\right]=0.
\]
Collect terms:
\[
...
Cross product:
\[
\Omega\hat{\vb y}\times\vb r_{\text{from axis}}=\Omega\!\left[\sqrt{a^2-x^2}\,\hat{\vb x}+x\,\hat{\vb z}\right].
\]
Apply $x\ll a$:
\[
\boxed{\;\vb u(x,z=h(x))\approx U\hat{\vb x},\qquad U=\Omega a\;}
\]
($\hat{\vb z}$ component is $O(\Omega x)\ll U$).
...
C(x)=0.
\]
Apply $u_x(z=h)=\Omega a$:
\[
B(x)=\frac{\Omega a}{h(x)}.
\]
Therefore
\begin{equation}
\boxed{\;u_x(x,z)\approx \frac{\Omega a\,z}{h(x)}\;}
\label{eq:lub-ux}
\end{equation}
(linear Couette across the gap, shear rate $\Omega a/h(x)$).

\subsection*{(c) Force and torque}

Viscous shear stress on the cylinder (in $\hat{\vb x}$):
\[
\tau_{xz}\big|_{z=h}=\mu\pd{u_x}{z}\bigg|_{z=h}=\frac{\mu\Omega a}{h(x)}.
\]
Net horizontal force on the cylinder (length $L$, integrated over the thin-gap strip $|x|<b$, sign such that the fluid drags the cylinder in $-\hat{\vb x}$):
\[
F\approx -\int_{-b}^{b}\frac{\mu\Omega a}{h(x)}L\,\de x=\int_{-b}^{b}\frac{A}{h}\de x,\qquad A=-\mu\Omega a L.
\]
Torque about the axis (lever arm $\approx a$):
\[
...
\]
Substitute $x=\sqrt{2ah_0}\,\tan\theta$, $\de x=\sqrt{2ah_0}\sec^2\theta\,\de\theta$, $h=h_0\sec^2\theta$:
\[
\int_{-\infty}^{\infty}\frac{\de x}{h(x)}=\frac{\sqrt{2ah_0}}{h_0}\int_{-\pi/2}^{\pi/2}\de\theta=\sqrt{\frac{2a}{h_0}}\,\pi.
\]
Therefore
\begin{equation}
...
\label{eq:F-Gamma}
\end{equation}
Both diverge as $h_0\to 0$ --- characteristic lubrication amplification.
...
\[
\vb v_{\text{surf}}=V\hat{\vb x}+\Omega\hat{\vb y}\times(-a\hat{\vb z})=(V-\Omega a)\hat{\vb x}.
\]
The plate is fixed: top BC becomes $u_x(z=h)=V-\Omega a$. The lubrication analysis is linear, so replace $\Omega a\to \Omega a-V$ in (c):
\[
...
\]
Pure rolling. Combination $MV+I\Omega/a$ is a conserved Casimir of the dynamics (no torque about the contact line from the lubrication shear when $s=0$).
