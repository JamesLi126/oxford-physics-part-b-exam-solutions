...
\]
Insert into \eqref{eq:FFsph}:
\[
F_{\rm sh}(q)=\frac{4\pi}{Ze\,q}\int_{0}^{\infty}r\cdot\frac{Ze}{4\pi R^{2}}\delta(r-R)\sin(qr)\,\de r.
\]
Delta picks $r=R$:
\[
...
Density $\rho_{\rm v}(r)=3Ze/(4\pi R^{3})$ for $r\le R$. Insert into \eqref{eq:FFsph}:
\[
F_{\rm v}(q)=\frac{4\pi}{Ze\,q}\cdot\frac{3Ze}{4\pi R^{3}}\int_{0}^{R}r\sin(qr)\,\de r.
\]
Cancel $4\pi/(Ze)$:
\[
...
\[
\int_{0}^{R}r\sin(qr)\,\de r=\left[-\frac{r\cos(qr)}{q}\right]_{0}^{R}+\frac{1}{q}\int_{0}^{R}\cos(qr)\,\de r.
\]
Evaluate boundary and remaining integral:
\[
\int_{0}^{R}r\sin(qr)\,\de r=-\frac{R\cos(qR)}{q}+\frac{\sin(qR)}{q^{2}}.
\]
Substitute:
\[
F_{\rm v}(q)=\frac{3}{qR^{3}}\cdot\frac{1}{q^{2}}\bigl[\sin(qR)-qR\cos(qR)\bigr].
\]
Collect powers of $qR$:
\begin{equation}
...
\]
Massless neutrino: $E_\nu=p_\nu c=Q-T_e$:
\[
p_\nu^{2}=(Q-T_e)^{2}/c^{2}.
\]
Eliminate $\de p_\nu$ via the delta ($\de p_\nu=\de E_\nu/c$):
\[
\frac{\de\Gamma}{\de p_e}\propto p_{e}^{2}(Q-T_{e})^{2}.
\]
Ultra-relativistic limit $T_e\to E_e=p_e c$:
\[
\frac{\de\Gamma}{\de p_e}\propto p_{e}^{2}(Q-p_{e}c)^{2}.
\]
Integrate over allowed range $0\le p_e c\le Q$:
\[
\Gamma\propto \int_{0}^{Q/c}p_{e}^{2}(Q-p_{e}c)^{2}\de p_{e}.
\]
Rescale $x=p_e c/Q$, $\de p_e=(Q/c)\de x$:
\[
\Gamma\propto \frac{Q^{2}}{c^{2}}\cdot Q^{2}\cdot \frac{Q}{c}\int_{0}^{1}x^{2}(1-x)^{2}\,\de x.
\]
Collect powers of $Q$:
\[
\Gamma\propto Q^{5}\int_{0}^{1}x^{2}(1-x)^{2}\,\de x.
\]
The integral evaluates to $1/30$, a pure number:
\begin{equation}
...
Neutron (BR$=1$):
\[
\Gamma_n=\frac{\hbar}{\tau_n}=\frac{6.582\times 10^{-22}}{879.4}\,\si{MeV}.
\]
Evaluate:
\[
\Gamma_n=\SI{7.49e-25}{MeV}.
\]
$\Lambda^{0}$ (BR$=8.3\times 10^{-4}$):
\[
\Gamma_\Lambda=\frac{8.3\times 10^{-4}\cdot 6.582\times 10^{-22}}{2.63\times 10^{-10}}\,\si{MeV}.
\]
Evaluate:
\[
\Gamma_\Lambda=\SI{2.08e-15}{MeV}.
\]
$Q$-values from $Q\simeq m_B-m_p-m_e$ ($m_p=938.3$, $m_e=0.51$, $m_n=939.6$, $m_\Lambda=1115.7$ MeV):
\[
Q_n=939.6-938.3-0.51=\SI{0.78}{MeV}.
\]
\[
Q_\Lambda=1115.7-938.3-0.51=\SI{176.9}{MeV}.
\]
Compute $Q$-ratio:
\[
\frac{Q_n}{Q_\Lambda}=\frac{0.78}{176.9}=4.41\times 10^{-3}.
\]
Raise to fifth power:
\[
(4.41\times 10^{-3})^{5}=1.66\times 10^{-12}.
\]
Compute width ratio:
\[
...
Multiply:
\[
\tan^{2}\theta_{C}=2.78\times 10^{9}\cdot 1.66\times 10^{-12}=4.61\times 10^{-3}.
\]
Take square root:
\[
\tan\theta_{C}=\sqrt{4.61\times 10^{-3}}=0.0679.
\]
Take arctangent:
\begin{equation}
\boxed{\;\theta_{C}\approx \arctan(0.068)\approx 3.9^{\circ}.\;}
\label{eq:thetaC-naive}
\end{equation}

\textbf{Comment.} Accepted value $\theta_C\approx 13^\circ$; the simple $Q^5$ rule under-estimates by $\sim 3$ because $Q_n\sim m_e$ violates the ultra-relativistic limit. Exact Fermi-integral $f_n$ exceeds $Q_n^5/30$ by $\sim 9$, restoring $\theta_C\approx 13^\circ$. The naive estimate \eqref{eq:thetaC-naive} is the answer extractable from the table alone.
...
Differentiate $s=m_p^{2}+2m_pE_\gamma$:
\[
2W\,\de W=2m_p\,\de E_\gamma\;\Longrightarrow\;\frac{\de W}{\de E_\gamma}=\frac{m_p}{W}.
\]
Convert FWHM to width in $W$:
\[
\Gamma=\frac{m_p}{W}\Gamma_{\rm lab}=\frac{0.938}{1.22}\cdot 0.20\,\si{GeV}.
\]
Evaluate:
\[
\Gamma\approx \SI{0.15}{GeV}=\SI{150}{MeV}.
\]
Lifetime from $\tau=\hbar/\Gamma$ with $\hbar=\SI{6.582e-22}{MeV.s}$:
\[
\tau=\frac{6.582\times 10^{-22}\,\si{MeV.s}}{150\,\si{MeV}}.
\]
Evaluate:
\[
\tau\approx \SI{4.4e-24}{s}.
\]
\begin{equation}
...
Breit--Wigner at $E=E_0$ gives factor $4\Gamma_i\Gamma_f/\Gamma^{2}$; sum over final states ($\sum_f\Gamma_f=\Gamma$):
\[
\sigma_{\max}=\frac{2J+1}{(2s_1+1)(2s_2+1)}\frac{\pi}{k^{2}}\,\frac{4\Gamma_i\Gamma}{\Gamma^{2}}.
\]
Simplify:
\[
\sigma_{\max}=\frac{2J+1}{(2s_1+1)(2s_2+1)}\frac{4\pi}{k^{2}}\,\frac{\Gamma_i}{\Gamma},\qquad \Gamma_i=\Gamma_{\gamma p}.
\]
...
\label{eq:Bgammap}
\end{equation}
Consistent with PDG value $\sim 0.6\%$ for $\Delta\to N\gamma$.

\subsection*{(d) Flat plateau above $\SI{2}{GeV}$}

Reduced wavelength $\bar\lambda=\hbar c/E_\gamma\lesssim \SI{0.1}{fm}$ at $E_\gamma\gtrsim\SI{2}{GeV}$ resolves individual quarks/gluons; cross section becomes the incoherent sum of pointlike parton-level scatters. No further resonance peaks because the photon couples to free partons rather than exciting collective $qqq$ states. The constant $\sigma\sim\SI{0.13}{mb}$ is the Bjorken-scaling/Pomeron-dominated total cross section, characteristic of incoherent point-like scattering.
...
\]
Compute $\partial_A B$ ($\partial_A$ of each term of $B$):
\[
...
Form $-4\partial_A B$:
\[
-4\partial_A B=-4\alpha+\frac{8\beta}{3A^{1/3}}-\frac{4\gamma(A-2Z)^{2}}{A^{2}}+\frac{8\gamma(A-2Z)}{A}-\frac{4\epsilon Z^{2}}{3A^{4/3}}.
\]
Form $-2\partial_Z B$:
\[
-2\partial_Z B=-\frac{8\gamma(A-2Z)}{A}+\frac{4\epsilon Z}{A^{1/3}}.
\]
Add; the $\pm 8\gamma(A-2Z)/A$ terms cancel:
\[
...
\[
0=\pd{M}{Z}=(m_p-m_n)-4\gamma\frac{A-2Z}{A}+\frac{2\epsilon Z}{A^{1/3}}.
\]
Neglect $m_p-m_n\ll \gamma$, rearrange:
\[
4\gamma\frac{A-2Z}{A}=\frac{2\epsilon Z}{A^{1/3}}.
\]
Multiply by $A$ and isolate $Z$:
\[
4\gamma A=Z\bigl(8\gamma+2\epsilon A^{2/3}\bigr).
\]
Divide:
\[
...
\textbf{Sanity check.} At $A=148$, $Z=61$, $A-2Z=26$: contributions $-34.9+9.2-2.9+32.9-4.5\approx -0.2$ MeV. Sign change at $A\sim 148$ confirmed.
