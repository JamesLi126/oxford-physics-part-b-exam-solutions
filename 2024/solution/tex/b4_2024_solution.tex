...
\]
Change variable $u=qr$:
\[
F(q)\;=\;\frac{3}{R^{3}q^{3}}\int_{0}^{qR}u\sin u\,\de u.
\]
Integrate by parts: $\int_{0}^{x}u\sin u\,\de u=\sin x-x\cos x$:
\begin{equation}
...
Small-$qR$ expansion: $\sin(qR)-qR\cos(qR)=(qR)^{3}/3-(qR)^{5}/30+\dots$:
\[
...
\[
q\;\gtrsim\;\frac{4.49\times \SI{197}{MeV.fm}}{\SI{3}{fm}}\;\approx\;\SI{295}{MeV/c}.
\]
Ultra-relativistic backscattering: $q_{\max}\simeq 2p\approx 2E/c$:
\[
E_{\min}\;\simeq\;\tfrac{1}{2}q_{\max}c\;=\;\tfrac{1}{2}\times\SI{295}{MeV}.
\]
\begin{equation}
...
\[
r_{\min}\;=\;\frac{197}{469.5\times 0.73}\,\text{fm}\;=\;\frac{197}{342.7}\,\text{fm}.
\]
\begin{equation}
...
Stationary point:
\[
\pd{BE}{Z}\bigg|_{A}\;=\;-\frac{2a_{C}Z}{A^{1/3}}+a_{A}\frac{A-2Z}{A}-(m_{n}-m_{p})c^{2}\;=\;0.
\]
Solve for $Z$:
\begin{equation}
...
Parent $M$ at rest, single daughter $m$. Energy: $E_{f}=Mc^{2}$. Momentum: $\vb p_{f}=0$, hence $E_{f}=mc^{2}$. Equate:
\[
M\;=\;m.
\]
For $M\neq m$ impossible; require $\geq 2$ daughters. Lightest pair for $\pi^{0}$: $\gamma\gamma$.
...
\paragraph{Why BR$(\eta\to\gamma\gamma)<$BR$(\pi^{0}\to\gamma\gamma)$.} The partial $\gamma\gamma$ widths are comparable, but the $\eta$ has additional open channels (e.g.\ $3\pi^{0}$, $\pi^{+}\pi^{-}\pi^{0}$, $\pi^{+}\pi^{-}\gamma$) closed for the lighter $\pi^{0}$. Hence $\Gamma_{\eta}\approx\SI{1.3}{keV}\gg\Gamma_{\pi^{0}}\approx\SI{8}{eV}$:
\begin{equation}
...
\begin{tikzpicture}[
  ...
  \draw[qline] (nu) -- (W2);
  \node[below=2pt] at ($(W2)!0.5!(nu)$) {$\bar\nu_{\ell}$};
...
\begin{tikzpicture}[
  ...
  \node[midway, right=2pt] at ($(V1)!0.5!(W1)$) {$W^{-}$};
  \draw[qline] (W1) -- (e1);
  \node[above=2pt] at ($(W1)!0.5!(e1)$) {$e^{-}/\mu^{-}/\bar u d/\bar c s$};
  \draw[qline] (nue1) -- (W1);
  \node[below=2pt] at ($(W1)!0.5!(nue1)$) {$\bar\nu_{e}/\bar\nu_{\mu}$};
...
Evaluate:
\[
\frac{\Gamma_{\tau}^{(e)}}{\Gamma_{\mu}}\;\approx\;7.55\times 10^{6}\times 0.178\;\approx\;1.34\times 10^{6}.
\]
Mass-ratio factor:
\[
\left(\frac{m_{\tau}}{m_{\mu}}\right)^{5}\;=\;\left(\frac{1777.0}{105.66}\right)^{5}\;=\;(16.82)^{5}.
\]
Evaluate:
\[
\left(\frac{m_{\tau}}{m_{\mu}}\right)^{5}\;\approx\;1.347\times 10^{6}.
\]
Take the ratio:
\[
\left(\frac{g_{\tau}}{g_{\mu}}\right)^{2}\;=\;\frac{1.34\times 10^{6}}{1.347\times 10^{6}}\;\approx\;0.995.
\]
\begin{equation}
