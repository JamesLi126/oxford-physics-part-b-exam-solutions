...
For $\psi=-\tfrac12\Omega(x^2+y^2)$, apply \eqref{eq:streamdef}:
\[ v_x = -\Omega y,\qquad v_y = \Omega x. \]
In polar coordinates: $\vb v=\Omega r\,\hat{\bm\theta}$, i.e.\ rigid rotation at rate $\Omega$. Apply \eqref{eq:psivort}:
\[ \omega = -\Lap\!\left[-\tfrac12\Omega(x^2+y^2)\right] = 2\Omega. \]
\begin{equation}
\boxed{\;\omega = 2\Omega.\;}
\end{equation}
...
Inviscid Navier--Stokes \eqref{eq:NS} with $\nu=0$, $\rho=$ const. Vector identity for irrotational flow:
\[ (\vb v\cdot\nabla)\vb v = \tfrac12\nabla|\vb v|^2 - \vb v\times(\nabla\times\vb v) = \tfrac12\nabla|\nabla\phi|^2. \]
Substitute \eqref{eq:phidef} and use $g\hat{\vb z}=\nabla(gz)$:
...
Kinematic BC $\D(z-\eta)/\D t = 0$ at $z=\eta$:
\begin{equation}
\pd{\eta}{t} + v_x\pd{\eta}{x} = v_z \qquad \text{at }z=\eta.
\label{eq:KBC-exact}
\end{equation}
Linearise (drop $v_x\partial_x\eta$, evaluate at $z=0$):
\begin{equation}
\pd{\phi}{z}\bigg|_{z=0} = \pd{\eta}{t}.
\label{eq:KBC-lin}
\end{equation}
Insert $\eta=A\cos(kx-\omega t)$:
\begin{equation}
\pd{\phi}{z}\bigg|_{z=0} = \omega A\sin(kx-\omega t).
\label{eq:KBC-explicit}
\end{equation}

Dynamic BC: pressure on the surface equals atmospheric (absorb into $C$). Evaluate \eqref{eq:bernoulli} at $z=\eta$, drop $\tfrac12|\nabla\phi|^2$ as second order, and Taylor to $z=0$:
\begin{equation}
\pd{\phi}{t}\bigg|_{z=0} + g\eta = 0.
\label{eq:DBC-lin}
\end{equation}

Deep water: $\phi\to 0$ as $z\to-\infty$. Take $\phi = F(z)\sin(kx-\omega t)$ in $\Lap\phi=0$:
\[ F''(z) - k^2 F(z) = 0. \]
Decaying solution ($k>0$):
\[ F(z) = B\,\mathrm{e}^{kz}. \]
Apply \eqref{eq:KBC-explicit}:
\[ kB = \omega A. \]
Solve for $B$:
\[ B = \frac{\omega A}{k}. \]
So
...
Substitute into \eqref{eq:DBC-lin}:
...
Phase and group speeds from \eqref{eq:disp}:
\[ c_\text{ph} = \frac{\omega}{k} = \sqrt{\frac{g}{k}},\qquad c_\text{gr} = \dv{\omega}{k} = \tfrac12\sqrt{\frac{g}{k}} = \tfrac12 c_\text{ph}. \]
...
From \eqref{eq:phi-soln}, gradient at $z=0$:
\[ v_x = \pd{\phi}{x}\bigg|_{z=0} = \omega A\cos(kx-\omega t). \]
\[ v_z = \pd{\phi}{z}\bigg|_{z=0} = \omega A\sin(kx-\omega t). \]

Lagrangian trajectory at leading order: integrate $\dot x = v_x(x_0,0,t)$, $\dot z = v_z(x_0,0,t)$:
\[ x(t)-x_0 = -A\sin(kx_0-\omega t). \]
\[ z(t)-z_0 = A\cos(kx_0-\omega t). \]
Square and add:
\[ [x-x_0]^2 + [z-z_0]^2 = A^2. \]
...
Vorticity in the $(x,z)$-plane:
\[ \omega_y = \pd{v_x}{z}-\pd{v_z}{x} = k\omega A\cos(kx-\omega t) - k\omega A\cos(kx-\omega t). \]
Cancel:
\begin{equation}
\boxed{\;\omega_y = 0.\;}
\end{equation}
...
Integrate \eqref{eq:thinfilm} twice in $z$ (treat $\partial_r p$ as $z$-independent):
...
Integrate continuity from $z'=0$ ($v_z=0$ at the table) to $z$:
...
Pressure BCs: $p(R)=p_0$ at outer rim, $p(a)=p_0$ at hole:
\[ p_0 = \frac{3\rho\nu\dot h}{h^3}R^2 + C_1\ln R + C_2. \]
...
Set $a=0$: the second bracket of \eqref{eq:p-final} vanishes (since $(R^2-a^2)/\ln(a/R)\to 0$):
...
Insert numbers. Dynamic viscosity:
\[ \mu = \rho\nu = \SI{e-3}{Pa.s}. \]
Numerator:
\[ 3\pi\,\mu\,\dot h\,R^4 = 3\pi\times(\SI{e-3}{Pa.s})\times(\SI{e-3}{m.s^{-1}})\times(\SI{1}{m^4}) = 3\pi\times 10^{-6}~\text{N\,m}^3. \]
Denominator:
\[ 2h^3 = 2\times(\SI{5e-4}{m})^3 = \SI{2.5e-10}{m^3}. \]
Divide:
\[ F = \frac{3\pi\times 10^{-6}}{2.5\times 10^{-10}}~\text{N} \approx \SI{3.77e4}{N}. \]
\begin{equation}
...
Read off:
\[ p' = \gamma\frac{p_0}{\rho_0}\rho' = c^2\rho',\qquad c^2 \equiv \pd{p}{\rho}\bigg|_s = \gamma\frac{p_0}{\rho_0}. \]
Differentiate \eqref{eq:lin-cont} in $t$:
\[ \pdd{\rho'}{t} + \rho_0\pd{}{x}\pd{v_x'}{t} = 0. \]
Substitute \eqref{eq:lin-mom} and $p'=c^2\rho'$:
\begin{equation}
\pdd{p'}{t} - c^2\pdd{p'}{x} = 0.
\label{eq:wave-eqn}
\end{equation}

Plane wave $p'\propto\mathrm{e}^{\mathrm{i}(kx-\omega t)}$ in \eqref{eq:wave-eqn}: $\omega^2=c^2 k^2$, hence
\[ c_\text{ph} = \omega/k = c,\qquad c_\text{gr} = \de\omega/\de k = c. \]
\begin{equation}
\boxed{\;c_\text{ph}=c_\text{gr}=c=\sqrt{\dv{p}{\rho}}.\;}
\end{equation}

Insert standard atmosphere:
\[ c^2 = \tfrac{7}{5}\cdot\frac{10^5}{1.2}~\text{m}^2\text{s}^{-2} = \frac{7\times 10^5}{6}~\text{m}^2\text{s}^{-2}. \]
Square root:
...
Define $\de P = \de p/(\rho c)$. For the adiabat $c^2=\gamma p/\rho$, so $c\propto\rho^{(\gamma-1)/2}$ and $P(\rho)$ is single-valued. Rewrite $\rho^{-1}\partial_x p$:
...
\end{equation}
Divide continuity by $\rho/c$ using $\de\rho = (\rho/c)\,\de P$:
\begin{equation}
\pd{P}{t} + v_x\pd{P}{x} + c\,\pd{v_x}{x} = 0.
\label{eq:cont-Riemann}
\end{equation}
Add \eqref{eq:mom-Riemann}+\eqref{eq:cont-Riemann}:
\begin{equation}
...
Estimate the breaking time. Two close characteristics carrying $v_x$ and $v_x+\Delta v_x$ separated by $\Delta x$ converge at relative speed $\Delta v_x$:
\[ \tau \sim \frac{\Delta x}{\Delta v_x}. \]
Use $\Delta v_x \sim \Delta x\,\partial_x v_x$ at the steepest-descent point ($\partial_x v_x|_\text{max}=-kv_0$):
\[ \tau_\text{break} \sim \frac{1}{kv_0}. \]
The implicit-solution analysis gives prefactor unity:
\begin{equation}
