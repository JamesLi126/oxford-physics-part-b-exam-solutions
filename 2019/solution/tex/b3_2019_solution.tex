...
Within a term, write the spin--orbit operator:
\[
H_{\rm SO}=\sum_i\xi(r_i)\,\bm l_i\!\cdot\!\bm s_i.
\]
Project onto the term (Land\'e projection theorem):
\[
H_{\rm SO}\big|_{^{2S+1}L}=\beta(L,S)\,\bm L\!\cdot\!\bm S.
\]
Couple $\bm J=\bm L+\bm S$ and square:
\[
\bm J^{2}=\bm L^{2}+\bm S^{2}+2\,\bm L\!\cdot\!\bm S.
\]
Solve for $\bm L\cdot\bm S$:
\[
...
\]
Form splitting between adjacent levels $J$ and $J-1$:
\[
...
Estimate $E_{L_1}$ from $KL_1L_3$ assignment:
\[
...
\]
Cross-check via $KL_1L_2=\SI{18.088}{keV}$:
\[
E_{L_1}=25.514-3.524-18.088=\SI{3.902}{keV}.
\]
Cross-check via $KL_1L_1=\SI{17.793}{keV}$:
\[
E_{L_1}=\tfrac12(25.514-17.793)=\SI{3.861}{keV}.
\]
Apply the same $\sim\SI{130}{eV}$ relaxation correction:
\[
...
Substitute $\cos(\omega t)=\tfrac12(e^{i\omega t}+e^{-i\omega t})$ into the given $\dot c_2$ equation:
\[
\dot c_2=-i\Omega_R c_1\,e^{i\omega_0 t}\cdot\tfrac12(e^{i\omega t}+e^{-i\omega t}).
\]
...
\label{eq:c2dot}
\end{equation}
Repeat the same steps on the $c_1$ equation:
\begin{equation}
\dot c_1=-\tfrac{i}{2}\Omega_R\,e^{i\delta\omega t}\,c_2.
\label{eq:c1dot}
\end{equation}

Differentiate \eqref{eq:c2dot} with respect to $t$:
\[
...
\[
\ddot c_2=(-i\delta\omega)\bigl(-\tfrac{i}{2}\Omega_R\,e^{-i\delta\omega t}c_1\bigr)-\tfrac{i}{2}\Omega_R\,e^{-i\delta\omega t}\,\dot c_1.
\]
Identify the bracket as $\dot c_2$ via \eqref{eq:c2dot}:
\[
...
\]
Collapse the exponentials $e^{-i\delta\omega t}e^{i\delta\omega t}=1$:
\[
\ddot c_2=-i\delta\omega\,\dot c_2+\bigl(-\tfrac{i}{2}\bigr)\bigl(-\tfrac{i}{2}\bigr)\Omega_R^{2}\,c_2.
\]
Evaluate $(-i/2)(-i/2)=-1/4$:
\[
...
Try the ansatz $c_2=Ae^{i\lambda t}$:
\[
...
\]
Define the effective Rabi frequency:
\[
\boxed{\;\Omega_{\rm eff}=\sqrt{\Omega_R^{2}+\delta\omega^{2}}.\;}
\]
Write the two roots as $\lambda_\pm=-\delta\omega/2\pm\Omega_{\rm eff}/2$ and form the general solution:
\[
...
B=0.
\]
Differentiate the ansatz at $t=0$:
\[
\dot c_2(0)=A\,\Omega_{\rm eff}/2.
\]
Evaluate \eqref{eq:c2dot} at $t=0$ with $c_1(0)=1$:
\[
\dot c_2(0)=-\tfrac{i}{2}\Omega_R.
\]
Equate the two expressions for $\dot c_2(0)$:
\[
...
\]
Take modulus squared (the phase factor has unit modulus):
\begin{equation}
...
Time-average \eqref{eq:rabi} using $\overline{\sin^{2}}=1/2$ (Lorentzian in $\delta\omega$):
\[
...
\paragraph{Broad-band weak field.} Treat the spectrum as a sum of independent monochromatic components of spectral energy density $\rho(\omega)$, each with weak Rabi frequency $\Omega_R(\omega)\propto\sqrt{\rho(\omega)}$. Sum incoherent contributions of \eqref{eq:rabi}:
\[
P_{1\to 2}(t)=\int\Bigl(\frac{\Omega_R(\omega)}{\Omega_{\rm eff}}\Bigr)^{2}\sin^{2}\!\bigl(\Omega_{\rm eff}t/2\bigr)\,\de\omega.
\]
Take the weak-field limit $\Omega_R\ll|\delta\omega|$ so that $\Omega_{\rm eff}\to|\delta\omega|$:
\[
P_{1\to 2}(t)\approx\int\Omega_R^{2}(\omega)\,\frac{\sin^{2}(\delta\omega\, t/2)}{\delta\omega^{2}}\,\de\omega.
\]
Pull the slowly-varying $\Omega_R^{2}(\omega)$ out at line centre and use $\int_{-\infty}^{\infty}\sin^{2}(x t/2)/x^{2}\,\de x=\pi t/2$:
\[
P_{1\to 2}(t)\propto\tfrac{\pi}{2}\,t\,\Omega_R^{2}(\omega_0).
\]
...
Write rate equations in the absence of stimulated emission:
\[
...
\]
Impose steady state $\dot N_2=0$:
\[
N_2=R_2\tau_2.
\]
Impose steady state $\dot N_1=0$:
\[
N_1=\tau_1(R_1+A_{21}N_2).
\]
Substitute the steady-state $N_2$:
\[
N_1=\tau_1 R_1+A_{21}\tau_1 R_2\tau_2.
\]
Form the inversion $N^{*}=N_2-N_1$ (equal degeneracies):
\[
N^{*}(0)=R_2\tau_2-R_1\tau_1-A_{21}\tau_1 R_2\tau_2.
\]
Factor out $R_2\tau_2$ from the first and third terms:
\begin{equation}
...
Define the stimulated rate per atom:
\[
W_{21}=\frac{\sigma_{21}(\omega)\,I}{\hbar\omega}.
\]
Add stimulated terms to the rate equations:
\[
...
Impose steady state $\dot N_2=0$:
\[
N_2=\tau_2\bigl[R_2-W_{21}N^{*}\bigr].
\]
Impose steady state $\dot N_1=0$:
\[
N_1=\tau_1\bigl[R_1+A_{21}N_2+W_{21}N^{*}\bigr].
\]
Subtract to form $N^{*}=N_2-N_1$:
\[
N^{*}=\tau_2 R_2-\tau_2 W_{21}N^{*}-\tau_1 R_1-A_{21}\tau_1 N_2-\tau_1 W_{21}N^{*}.
\]
Substitute $N_2=\tau_2(R_2-W_{21}N^{*})$ into the $A_{21}\tau_1 N_2$ term:
\[
N^{*}=\tau_2 R_2-\tau_1 R_1-A_{21}\tau_1\tau_2 R_2+A_{21}\tau_1\tau_2 W_{21}N^{*}-(\tau_1+\tau_2)W_{21}N^{*}.
\]
Group the $N^{*}(0)$ terms using \eqref{eq:Nstar0}:
\[
N^{*}=N^{*}(0)-W_{21}N^{*}\bigl[\tau_1+\tau_2-A_{21}\tau_1\tau_2\bigr].
\]
Define the recovery time $\tau_R\equiv \tau_1+\tau_2-A_{21}\tau_1\tau_2$:
\[
N^{*}\bigl(1+W_{21}\tau_R\bigr)=N^{*}(0).
\]
Solve for $N^{*}$:
\[
N^{*}=\frac{N^{*}(0)}{1+W_{21}\tau_R}.
\]
Substitute $W_{21}=\sigma_{21}I/(\hbar\omega)$ and write the denominator as $1+I/I_s$:
\begin{equation}
\boxed{\;N^{*}(I)=\frac{N^{*}(0)}{1+I/I_s(\omega)},\qquad I_s(\omega)=\frac{\hbar\omega}{\sigma_{21}(\omega)\,\tau_R}.\;}
\label{eq:Is}
\end{equation}

\paragraph{Importance and frequency dependence.} $I_s$ sets the intensity at which an amplifier transitions from linear (small-signal) gain to logarithmic (saturated) gain. Above $I_s$ the gain $\gamma(\omega)=\sigma_{21}(\omega)N^{*}(0)/(1+I/I_s)$ falls; this caps the achievable single-pass amplification and fixes the energy extraction efficiency.
...
Write the saturated propagation equation:
\[
\frac{\de I}{\de z}=\frac{\gamma_0(\omega)\,I}{1+I/I_s(\omega)}.
\]
Multiply through by $(1+I/I_s)/I$:
\[
\Bigl(\frac{1}{I}+\frac{1}{I_s}\Bigr)\de I=\gamma_0\,\de z.
\]
Integrate from $z=0,\,I=I_0$ to $z=L,\,I=I_{\rm out}$:
\begin{equation}
...
Write \eqref{eq:rigrod} at $\omega_1$ with $I_{\rm out}=3.2I_0$:
\[
\ln(3.2)+(3.2-1)\frac{I_0}{I_s(\omega_1)}=\gamma_0(\omega_1)L.
\]
Write \eqref{eq:rigrod} at $\omega_2$, using $I_s(\omega_2)=2I_s(\omega_1)$ and $\gamma_0(\omega_2)=\tfrac12\gamma_0(\omega_1)$:
\[
...
\]
Subtract from the first to eliminate $\gamma_0(\omega_1)L$:
\[
\ln(3.2)-2\ln(2.1)+\bigl[(3.2-1)-(2.1-1)\bigr]\frac{I_0}{I_s(\omega_1)}=0.
\]
Evaluate the bracket $2.2-1.1=1.1$:
\[
\ln(3.2)-2\ln(2.1)+1.1\,\frac{I_0}{I_s(\omega_1)}=0.
\]
Evaluate the logarithms:
\[
\ln(3.2)=1.1632,\qquad 2\ln(2.1)=2(0.7419)=1.4839.
\]
Insert and isolate $I_0/I_s(\omega_1)$:
\[
1.1\,\frac{I_0}{I_s(\omega_1)}=1.4839-1.1632=0.3207.
\]
Divide by $1.1$:
\[
\frac{I_0}{I_s(\omega_1)}=\frac{0.3207}{1.1}=0.2916.
\]
Invert and use $I_s(\omega_2)=2I_s(\omega_1)$:
\[
\boxed{\;I_s(\omega_1)=3.43\,I_0,\qquad I_s(\omega_2)=6.86\,I_0.\;}
\]

\paragraph{Small-signal gain.} Substitute back into the $\omega_1$ equation:
\[
\gamma_0(\omega_1)L=\ln(3.2)+2.2\,(0.2916).
\]
Evaluate:
\[
\gamma_0(\omega_1)L=1.1632+0.6415=1.805.
\]
Divide by $L=\SI{50}{mm}=\SI{0.050}{m}$:
\[
\gamma_0(\omega_1)=1.805/0.050=\SI{36.1}{m^{-1}}.
\]
Apply $\gamma_0(\omega_2)=\tfrac12\gamma_0(\omega_1)$:
\[
...
\paragraph{Consistency check at $\omega_2$.} Apply \eqref{eq:rigrod} with $I_{\rm out}=2.1I_0$:
\[
\gamma_0(\omega_2)L=\ln(2.1)+(2.1-1)\,\frac{I_0}{I_s(\omega_2)}.
\]
Substitute $I_s(\omega_2)=6.86I_0$:
\[
\gamma_0(\omega_2)L=0.7419+1.1\times(1/6.86).
\]
Evaluate:
\[
\gamma_0(\omega_2)L=0.7419+0.1604=0.902.
\]
Divide by $L$:
\[
