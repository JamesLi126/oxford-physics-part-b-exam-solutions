...
Both have $(ns)^2$ ground giving $^1S_0$; lowest excited configurations are $ns(n-1)d$ and $nsnp$. In X, $ns(n-1)d$ lies \emph{below} $nsnp$; in Y the order is reversed and the inset shows $\sim 10^3\,\si{cm^{-1}}$ fine structure. In heavy atoms $(n-1)d$ is pulled in and fine structure $\sim Z^4\alpha^2$ is large, so
\begin{equation}
...
\paragraph{Zero field.} $2\vb s_e\!\cdot\!\vb s_p=\vb S_{\rm tot}^2-\vb s_e^2-\vb s_p^2$ gives
\[
\langle\vb s_e\!\cdot\!\vb s_p\rangle = \tfrac{\hbar^2}{2}[S_{\rm tot}(S_{\rm tot}+1)-\tfrac32]=\begin{cases}+\tfrac14\hbar^2,& S_{\rm tot}=1\\ -\tfrac34\hbar^2,& S_{\rm tot}=0\end{cases}
\]
\begin{equation}
...
\paragraph{High field.} Product basis: $H_Z=2\mu_B B\hbar(m_{s_e}-m_{s_p})$; diagonal hyperfine $\langle\vb s_e\!\cdot\!\vb s_p\rangle\to\hbar^2 m_{s_e}m_{s_p}$ (drop off-diagonal $s_+s_-$):
\begin{equation}
\boxed{\;\begin{aligned}
...
\begin{tikzpicture}[>=stealth, scale=1.0]
  ...
  \node[above right=2pt] at (5.6,3.0) {$2\mu_B B \sim A\hbar^{2}$};
\end{tikzpicture}
\end{center}
...
For $\sqrt{\Omega^2+\delta^2}\,T\ll 1$, $\sin x\approx x$:
\begin{equation}
|c_2(T)|^{2} \approx \frac{(\Omega T)^{2}}{4},
\label{eq:smallT}
\end{equation}
\emph{independent of $\delta$}: time--energy uncertainty $\sim 1/T$ exceeds $\delta$, so all detunings within the pulse bandwidth are excited equally.
...
\]
Hence $\cos^{2}(\cdots) = 1 + \mathcal O(\delta^{4}/\Omega^{4})$. Combine with the prefactor:
\[
P_2(\delta) \approx \left(1 - \tfrac{\delta^{2}}{\Omega^{2}}\right)\!\left(1 + \mathcal O(\delta^{4}/\Omega^{4})\right) = 1 - \frac{\delta^{2}}{\Omega^{2}}.
\]
Uniform $\delta\in[-\Delta/2,\Delta/2]$, $\langle\delta^2\rangle=\Delta^2/12$:
\begin{equation}
...
N_2 = \frac{s}{1+s}\,N_1.
\]
Use total population $N_1+N_2=N_T$:
\[
...
At $T=\SI{10}{K}$:
\[
\Delta\nu_D(\SI{10}{K})\approx \SI{32}{MHz}.
\]
At $T=\SI{1}{mK}$ (scales as $\sqrt T$, factor $10^{-2}$):
\[
\Delta\nu_D(\SI{1}{mK})\approx \SI{0.32}{MHz}.
\]

At $\SI{1}{mK}$: natural-dominated Lorentzian, FWHM $\approx \SI{5.9}{MHz}$, $\sigma\approx\sigma_{\rm peak}$. Compute optical depth:
\[
...
At $\SI{10}{K}$: Doppler-dominated Gaussian, FWHM $\approx \SI{32}{MHz}$. Peak $\sigma$ reduced by $\Gamma_{\rm nat}/\Delta\omega_D\approx 0.18$:
\[
N_T\sigma L \approx 3\times 0.18 \approx 0.54.
\]
Transmission:
\[
e^{-0.54}\approx 58\%.
...
\begin{tikzpicture}[>=stealth, scale=1.0]
  ...
  % T = 1 mK : narrow Lorentz, deep dip
  \draw[thick,blue,domain=0:10,samples=240]
       plot (\x, {3 - 2.85/(1+((\x-5)/0.4)^2)});

  % T = 10 K : broad Gauss, shallow dip
  \draw[thick,red,domain=0:10,samples=240]
       plot (\x, {3 - 1.26*exp(-((\x-5)/2)^2*ln(2)*4)});
  ...
  \fill[red] (5,1.74) circle (1.6pt);
  \node[red, above right=2pt] at (6.6,1.78) {\small $\SI{10}{K}$ (Gauss, $\sim\!\SI{32}{MHz}$)};

  % x-axis tick marks; numeric labels BELOW the axis (outside)
  \foreach \x/\v in {1/-40,3/-20,5/0,7/20,9/40} {
    \draw (\x,0.08)--(\x,-0.08);
...
\begin{tikzpicture}[>=stealth, scale=1.0]
  ...
  % broad dip + narrow Lamb peak at line centre (with counter-pump)
  \draw[thick,green!55!black,domain=0:10,samples=320]
       plot (\x, {3 - 1.26*exp(-((\x-5)/2)^2*ln(2)*4)
                    + 0.6/(1+((\x-5)/0.35)^2)});

  % anchor markers and labels — both parked OUTSIDE their curves
  \fill[green!55!black] (5,2.34) circle (1.6pt);
  \node[green!55!black, above right=2pt] at (6.0,3.05)
       {\small with pump (Lamb dip)};

  \fill[red] (5,1.74) circle (1.6pt);
  \node[red, above right=2pt] at (6.0,1.55)
       {\small without pump};
