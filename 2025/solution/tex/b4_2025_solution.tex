...
\]
Multiply out $j_X N_Y$:
\[
j_X N_Y\;=\;\frac{I_X}{A}\cdot\rho A\,\Delta x\;=\;I_X\,\rho\,\Delta x.
\]
Cancel $A$:
\[
...
\]
Nucleon count from total mass $M=1.74\times 10^{7}\,\text{kg}$:
\[
N_\text{nuc}\;=\;\frac{M}{m_u}.
\]
Substitute $m_u=1.661\times 10^{-27}\,\text{kg}$:
\[
N_\text{nuc}\;=\;\frac{1.74\times 10^{7}}{1.661\times 10^{-27}}\;=\;1.048\times 10^{34}.
\]

For neutral matter $N_e/N_\text{nuc}=Z/A$:
\[
\frac{N_e}{N_\text{nuc}}\;=\;\frac{5.79\times 10^{29}}{1.048\times 10^{34}}\;=\;5.52\times 10^{-5}.
\]
...
\]
(The literal ratio $5.5\times 10^{-5}$ is anomalously small versus the candidates, indicating an internal scale issue in the problem data; among the three candidates the largest $Z/A$ and the only one used at kilo-tonne scale for solar neutrino detection (Super-K, $\sim 5\times10^{4}\,\text{t}$) is water.)
\begin{equation}
...
Surface bracket:
\[
236^{2/3}-235^{2/3}\;=\;38.189-38.081\;=\;0.1080.
\]
Multiply by $-\beta$:
\[
-\beta\,[236^{2/3}-235^{2/3}]\;=\;-17.23\times 0.1080\;=\;-1.861\,\text{MeV}.
\]
Coulomb bracket:
\[
236^{-1/3}-235^{-1/3}\;=\;0.16182-0.16205\;=\;-2.29\times 10^{-4}.
\]
Multiply by $-\epsilon Z^{2}$ with $Z^{2}=92^{2}=8464$:
\[
-\epsilon Z^{2}\,[236^{-1/3}-235^{-1/3}]\;=\;-0.697\times 8464\times(-2.29\times 10^{-4})\;=\;+1.351\,\text{MeV}.
\]
...
Multiply by $-\gamma$:
\[
-\gamma\,\bigl[52^{2}/236-51^{2}/235\bigr]\;=\;-23.285\times 0.3895\;=\;-9.070\,\text{MeV}.
\]
Pairing change:
\[
...
\[
\Delta B\;=\;15.56-1.861+1.351-9.070+0.781.
\]
\begin{equation}
...
Surface bracket:
\[
239^{2/3}-238^{2/3}\;=\;38.512-38.405\;=\;0.1075.
\]
Multiply by $-\beta$:
\[
-\beta\,[239^{2/3}-238^{2/3}]\;=\;-17.23\times 0.1075\;=\;-1.852\,\text{MeV}.
\]
Coulomb bracket:
\[
239^{-1/3}-238^{-1/3}\;=\;0.16114-0.16136\;=\;-2.25\times 10^{-4}.
\]
Multiply by $-\epsilon Z^{2}$:
\[
-\epsilon Z^{2}\,[239^{-1/3}-238^{-1/3}]\;=\;-0.697\times 8464\times(-2.25\times 10^{-4})\;=\;+1.327\,\text{MeV}.
\]
...
Sum the five contributions:
\[
\Delta B\;=\;15.56-1.852+1.327-9.426-0.778.
\]
\begin{equation}
...
\boxed{\;\eta=\nu p.\;}
\end{equation}
Evaluate at $E=2\,\text{MeV}$ ($p=0.12$):
\[
\eta_{2\,\text{MeV}}\;=\;2.5\times 0.12\;=\;0.30.
\]
Evaluate at $E=0.07\,\text{eV}$ ($p=0.79$):
\[
\eta_{0.07\,\text{eV}}\;=\;2.5\times 0.79\;=\;1.98.
\]
...
\[
\frac{\de\sigma}{\de\Omega}\;=\;\frac{2\pi}{\hbar}\,|M_{if}|^{2}\cdot\frac{V_\text{box}\,m k'}{(2\pi)^{3}\hbar^{2}}\cdot\frac{m V_\text{box}}{\hbar k}.
\]
Collect $\hbar$ and $2\pi$ factors:
\[
\frac{\de\sigma}{\de\Omega}\;=\;\frac{m^{2}V_\text{box}^{2}}{(2\pi)^{2}\hbar^{4}}\,\frac{k'}{k}\,|M_{if}|^{2}.
\]
...
\[
\tilde V(\bm q)\;=\;\int e^{-i\bm q\cdot\bm r}\frac{\alpha e^{-\mu r}}{r}\,\de^{3}\bm r\;=\;2\pi\alpha\int_0^{\infty}r e^{-\mu r}\!\left[\int_{-1}^{+1}e^{-iqru}\de u\right]\de r.
\]
Inner angular integral:
\[
...
\[
f(\bm k,\bm k')\;=\;-\frac{m}{2\pi\hbar^{2}}\cdot\frac{4\pi\alpha}{q^{2}+\mu^{2}}.
\]
Cancel the $2\pi$:
\begin{equation}
...
\]
Multiply numerator and denominator by $\hbar^{4}$ in the squared bracket:
\[
\frac{\de\sigma}{\de\Omega}\;=\;\frac{4m^{2}\alpha^{2}}{\bigl[\hbar^{2}(4k^{2}\sin^{2}(\theta/2)+\mu^{2})\bigr]^{2}}.
\]
Substitute $\hbar^{2}k^{2}=2mT$:
\[
...
Evaluate:
\begin{equation}
\boxed{\;\sqrt{s}\;\simeq\;11.5\,\text{GeV}.\;}
\end{equation}
Close to the $\Upsilon(4S)$ at $10.58\,\text{GeV}$ where BaBar/Belle/Belle II $B$-factories actually operate; the difference is reconciled by the asymmetric beam boost used in practice.
